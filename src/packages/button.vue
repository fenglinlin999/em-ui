<template>
  <button class="em-button" :class="btnClass" :disabled="loading" @click="$emit('click',$event)">
     <!--普通icon-->
     <em-icon :icon="icon" v-if="icon && !loading" class="icon"></em-icon>
     <!--带loading的icon-->
     <em-icon icon="loading" v-if="loading" class="icon"></em-icon>
     <!--文本内容-->
...
export default {
      ...
      iconPosition:{
        ...
        validator(type){
          if(!['left','right'].includes(type)){
             console.error('iconPosition的类型必须为:'+'left | right');
          }
          return true
        }
      },
    ...
    computed:{
      btnClass(){
        let classes = [];
        if(this.type){
           classes.push(`em-button-${this.type}`)
        }
        if(this.iconPosition){
           classes.push(`em-button-${this.iconPosition}`)
        }
        return classes;
      }
    }
