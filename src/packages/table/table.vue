            ...
            <thead>
               <tr>
                   <th v-for="(col,index) in cloneColumns" :key="index" :style="{width:col.width+'px'}">
                       ...
                       <div v-else class="em-table-cell">
                           <span>{{col.title}}</span>
                           <span v-if="col.sortable" class="sortable">
                               <em-icon icon="up" :class="{active:isAsc(col)}" @click="sort(col,isAsc(col)?'normal':'asc')"></em-icon>
                               <em-icon icon="down" :class="{active:isDesc(col)}" @click="sort(col,isDesc(col)?'normal':'desc')"></em-icon>
                           </span>
                        </div>
                   </th>
               </tr>
